...
genCTLSpec1 :: Int -> String
genCTLSpec1 x = "CTLSPEC\n   " ++ "AG" ++ paren("AF" ++ paren (andList (genStringList x x))) ++ ";\n"

genCTLSpec2 :: Int -> String
genCTLSpec2 x = "CTLSPEC\n   " ++ "AG" ++ paren("AF" ++ paren (andList (map ("!" ++)(genStringList x x)))) ++ ";\n"

genCTLSpec3 :: Int -> String
genCTLSpec3 x =   let
                     ant   = paren (andList (genStringList x x))
                     cons  = "AX" ++ (paren $ andList $ map ("!" ++) (genStringList x x))
                     imp   = paren (binIF ant cons)
                  in 
                     "CTLSPEC\n   " ++ "AG" ++ imp ++ ";"
